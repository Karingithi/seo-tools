...
export type ArticleFormProps = {
  ...
  setArticleTypeOpen: Dispatch<SetStateAction<boolean>>
  images: string[]
  addImage: () => void
...
export default function ArticleForm(props: ArticleFormProps) {
  const { fields, handleChange, renderError, articleTypeOpen, setArticleTypeOpen, images, addImage, removeImage, handleImageChange } = props
  ...
  return (
    <>
        ...
        <div className="tool-field">
          ...
          <div className="custom-select-wrapper article-select-wrapper relative" style={{ width: '100%' }}>
            <button
              ...
              onClick={() => setArticleTypeOpen((o: boolean) => !o)}
      ...
      <div className="tool-field">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded p-2 text-sm mt-0">{(fields.headline || '').length}/110 characters</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="tool-field relative">
          ...
          <DatePickerInput value={fields.datePublished} onChange={(iso) => handleChange('datePublished', iso)} placeholder="yyyy-mm-dd" />
          {renderError('datePublished')}
        </div>

        <div className="tool-field relative">
          <label className="tool-label">Date Modified</label>
          <DatePickerInput value={fields.dateModified} onChange={(iso) => handleChange('dateModified', iso)} placeholder="yyyy-mm-dd" />
